...
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    ...
                                    GhostBottleneck, GhostConv, Segment, CBAM, CA, C2fGhost, TA, GSConv, GSBottleneck, VoVGSCSP, FReLU, C3In, SimAM, C2fGSCSP, conv_bn_hswish, SELayer, MobileNet_Block, 
                                    CABottleneck, C2fCA, ECA, Conv_maxpool, ShuffleNetV2_InvertedResidual, SE)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
                ...
                BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, CBAM, CA, C2fGhost, TA, GSConv, GSBottleneck, VoVGSCSP, C3In, SimAM,
                C2fGSCSP, CABottleneck, C2fCA, ECA, MobileNet_Block, conv_bn_hswish, Conv_maxpool, ShuffleNetV2_InvertedResidual, SE}:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, C2fGhost, VoVGSCSP, C3In, C2fGSCSP, C2fCA}:
