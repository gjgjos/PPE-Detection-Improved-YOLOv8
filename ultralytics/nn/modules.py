...
class Conv(nn.Module):
    ...
    default_act = nn.SiLU() #nn.SiLU()  #nn.Hardswish() #nn.Mish() # default activation

    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        #self.act = FReLU(c2) if act else nn.Identity()
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        ...
        self.cv1 = Conv(c1, c_, k[0], 1)    #, act=FReLU(c_)
        self.cv2 = Conv(c_, c2, k[1], 1, g=g)   #, act=FReLU(c2)
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1):  # ch_in, ch_out, kernel, stride
        ...
        self.conv = nn.Sequential(
            ...
            nn.Identity(),  # dw DWConv(c_, c_, k, s, act=False) if s == 2 else nn.Identity()
...
class C2fGSCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.ModuleList((GSBottleneck( self.c,  self.c) for _ in range(n))) #self.c, self.c, e=1.0
...
class C2fCA(C2f):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.c = int(c2 * e)  # hidden channels
        self.m = nn.ModuleList(CABottleneck(self.c, self.c, shortcut) for _ in range(n))
