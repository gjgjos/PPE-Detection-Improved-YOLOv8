...
class yolov8_heatmap:
    ...
    def draw_detections(self, box, color, name, img):
        ...
        cv2.putText(img, str(name), (xmin, ymax - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, tuple(int(x) for x in color), 2, lineType=cv2.LINE_AA)
        return img
...
def get_params():
    params = {
        'weight': 'basic.pt', #train_add_ca_backbone1 train_without_pretrained_v1
        'cfg': '../custom_yolov8s.yaml', #custom_yolov8s_add_ca_backbone custom_yolov8s
        'device': 'cuda:0',
        'method': 'GradCAM', # GradCAMPlusPlus, GradCAM, XGradCAM
        'layer': 'model.model[-3]', #9, 13, 19, 22, 25
...
if __name__ == '__main__':
    ...
    model('all/crawling (60).jpg', 'gradcam_result_ca')